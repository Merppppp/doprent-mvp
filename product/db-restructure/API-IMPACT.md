# DB Restructure — API / Code Impact Map (Phase 4 input, rev 3)

Change numbers referenced below (full design in `DESIGN.md`):

| # | Schema change |
|---|---|
| 1 | ~~`occasions`: PK `key` → `id uuid`~~ **superseded by 19 — `occasions` table dropped entirely (rev 3)** |
| 2 | `areas`: PK `key` → `id uuid`; `shops.area_key` → `area_id` uuid FK (areas stay a separate table — host decision rev 3) |
| 3 | `dresses.boutique_name` **dropped** → join `shop.name` |
| 4 | `dresses.occasions text[]` → ~~`product_occasions`~~ **`product_tags` via the tag system (rev 3, see 19)** |
...
| **18** | **(rev 3) `product_categories` (hierarchical, per type) + `products.category_id` (nullable, exactly ONE category, SET NULL)** — new optional field on product create/edit; new browse/filter dimension. DESIGN §3.2. |
| **19** | **(rev 3) Tag system `tag_groups`/`tags`/`product_tags` REPLACES `occasions` + `product_occasions`** — occasions become tag_group `key='occasion'`; old occasion keys live on as tag keys (URLs like `/?occasion=wedding` keep working via tag-key lookup). Everything that read `occasions` reads the tag system now — see §occasions below. |

## §occasions — what the tag-system swap means for code (change 19)

The occasion **keys** survive unchanged (`wedding`, `graduation`, … are now `tags.key` in group `occasion`), so URL params, i18n keys (`occasion.*` in `lib/i18n.ts`), and seed vocabulary all keep working. What changes is *where the list comes from and how products link to it*:

| Concern | Old (rev 2 design) | New (rev 3) |
|---|---|---|
| List occasions for UI (landing row, filters, nav) | `db.occasion.findMany(orderBy sortOrder)` | `db.tag.findMany({ where: { tagGroup: { key: "occasion" }, isActive: true }, orderBy: { label: "asc" } })` — tags have **no `sort_order`** (host spec); if curated ordering matters, keep today's `FALLBACK_OCCASIONS` array order as a UI-side constant. |
| Product↔occasion link | `product_occasions` join | `product_tags` join; filter = `productTags: { some: { tag: { key: { in: keys } } } }` |
| Occasion fields `th`/`en` | columns on `occasions` | `tags.label` holds Thai; English keeps coming from `lib/i18n.ts` (`occasion.<key>`) — already the pattern for `en` today. |
| `color_token` (OccasionTile / chips) | column on `occasions` | **no tag-system column** — move to a UI constant `OCCASION_COLOR: Record<string, token>` keyed by tag key (default token for unknown keys). Lives next to `FALLBACK_OCCASIONS` in the catalog lib. |
| Picking occasions in seller form | string array checkboxes → `product_occasions` | same UI, submits tag keys → server resolves to `tags.id` and syncs `product_tags` (`deleteMany` + `createMany`). |
| Admin metrics by occasion | `unnest(d.occasions)` raw SQL | `JOIN product_tags pt JOIN tags t JOIN tag_groups tg ON tg.key='occasion'` (see Workstream B row). |

## Table R — rename map (change 15)

| Old model / accessor | New model / accessor | Old field names | New field names |
|---|---|---|---|
| `Dress` / `db.dress` | `Product` / `db.product` | `boutiqueId`, `boutique` | `shopId`, `shop` (+ new `productTypeId`, `productType`; rev 3: + `categoryId`, `category`, `productTags`) |
...
| `Occasion` / `db.occasion` *(rev 2)* | **dropped (rev 3)** → `Tag` / `db.tag` in group `occasion` | `th`, `en`, `colorToken`, `sortOrder` | `label` (Thai); `en`→i18n, `colorToken`→UI constant, no sort (see §occasions) |
| `DressOccasion`/`ProductOccasion` *(rev 2)* | **dropped (rev 3)** → `ProductTag` / `db.productTag` | `dressId`, `occasionId` | `productId`, `tagId`; compound key `productId_tagId` |
| — *(new, rev 3)* | `ProductCategory` / `db.productCategory`, `TagGroup` / `db.tagGroup`, `Tag` / `db.tag`, `ProductTag` / `db.productTag` | — | new accessors (taxonomy) |
...
| `lib/types.ts` | 8, 9, 11, 14, 15, 16, 18, 19 | bigint ids → string; `submitted_at` → `created_at`; `KycPlan` type lowercase; + `ProductType` type + `product_type_key` on the product shape; *(rev 3)* + `ProductCategory`/`TagGroup`/`Tag` types + optional `category_key` on the product shape; keep the public `Occasion` TS shape (`{ key, th, en, color_token }`) as the *mapper output* type — it is now assembled from tag + i18n + color constant, so pages don't churn. Keep `Dress`/`Boutique` public shapes otherwise UNCHANGED (renaming these TS types = host-optional, see strategy). |
| `lib/dresses.ts` | 1–6, 15, 16, 18, 19 | All queries: `db.dress`→`db.product`, `db.boutique`→`db.shop`, field renames per Table R. `mapDress`: include `images(orderBy sortOrder)`, `priceTiers(orderBy minDays)`, `productTags{tag.key}` *(rev 3 — emit as the existing `occasions: OccasionKey[]` field, filtered to the `occasion` group)*, `productType{key}`, `category{key}`, `shop{name, area{key}}` and flatten to the existing output shape (`boutique_name` ← `shop.name`). Search filter `boutiqueName contains` → `shop: { name: { contains } }`; occasions filter (was `hasSome`) → `productTags: { some: { tag: { key: { in } } } }`. **`listOccasions()` rewired to the tag system** (§occasions query) — return shape unchanged via `FALLBACK_OCCASIONS`-style mapping (th=label, en=i18n, color_token=UI constant); fallback array stays. Catalog default filter: `productType: { key: "dress" }` (preserves today's behavior until multi-type browse ships). Optional new `category` filter param (key-based) — additive. Seed-similarity scorer (`occasions` set intersection) keeps working off the mapped keys. |
...
| `prisma/seed.ts`, `prisma/seed.dev.ts` | 2, 4, 5, 6, 14, 15, 16, 18, 19 | Rewrite — **host-specified seed order: `product_types` → `product_categories` (dress tree + sample subcategories, suit tree) → `tag_groups`/`tags` (group `occasion` + tags from the current occasions seed data) → shops/products…** (full plan DESIGN §10.4). Areas keep uuid ids (`key` unique). Product seed connects `productType` by key `"dress"`, optionally sets `category` by key, creates child rows (`product_images`, `product_price_tiers`, `product_tags` via tag-key lookup); shops connect area by `key` lookup; KYC plan lowercase. |
...
| `app/browse`, `app/page.tsx` (landing incl. occasions row + `/?occasion=` filter), `components/BrowseFilters.tsx`, `components/NavCategoryRow.tsx`, `components/DressResults.tsx`, `components/DressCard.tsx`, `components/DressArt.tsx` (`OccasionTile`), `components/Footer.tsx`, `components/Gallery.tsx`, `components/DistanceBadge.tsx`, `app/boutiques/page.tsx`, `app/boutique/[slug]/page.tsx`, `app/loading.tsx` | 1–5, 19 (indirect) | Should need **no edits** if mappers preserve shapes — all occasion consumers (landing occasions row, `NavCategoryRow`, `BrowseFilters` occasion picker, `OccasionTile color_token`, footer occasion links) get their data from `listOccasions()` / `mapDress().occasions`, which now read the tag system but emit the same `Occasion` shape (§occasions). URL param `?occasion=<key>` keeps working — keys are preserved as tag keys. Verify + typecheck. Component/route names keep `Dress`/`boutique` (host-optional). DistanceBadge keeps receiving `area.key` via mapper. |
...
| File | Changes | Work |
|---|---|---|
| `app/actions/seller.ts` | 2–6, 12, 14, 15, 16, 18, 19 | `createBoutique`/`updateBoutique` → `db.shop` (function names: rename to `createShop` etc. recommended while editing anyway — internal symbols, cheap). `createDress`/`updateDress` → `db.product`: drop `boutiqueName`; **set `productTypeId` (resolve `db.productType.findUnique({ where: { key: "dress" } })` — seller UI is dress-only today)**; *(rev 3)* accept optional `category` key → resolve to `categoryId` (NULL ok); nested `create`/`deleteMany+createMany` for images/tiers; **occasion picker payload (string keys) → resolve via `db.tag.findMany({ where: { key: { in }, tagGroup: { key: "occasion" } } })` and sync `product_tags`** (unknown/inactive keys rejected — was silently accepted by the old text[]). `updateDressPriceTiers` → `db.productPriceTier` rewrite. KYC `plan` lowercase. Wrap mutations in `withActor`. |
...
| `app/sell/(authed)/dresses/{new/page.tsx,[id]/edit/page.tsx}`, `app/dress/[id]/edit/page.tsx`, `DressForm.tsx` | 4, 5, 6, 15, 18, 19 | Fetch via mapper (occasions as keys, images as urls); **occasion checkbox options come from `listOccasions()` (tag-backed) instead of a hard-coded list — form still submits plain keys, payload format unchanged**; *(rev 3, optional)* add category select fed by `db.productCategory` (dress tree, active only). File names keep `Dress*` (host-optional). |
...
| `app/admin/metrics/page.tsx` | 4, 8, 13, 15, 19 | Raw SQL: table names `dresses`→`products`, `boutiques`→`shops`, `seller_subscriptions`→`shop_subscriptions`, `dress_id`→`product_id`, `boutique_id`→`shop_id`; **by-occasion metric `unnest(coalesce(d.occasions, …))` → `JOIN product_tags pt ON pt.product_id = p.id JOIN tags t ON t.id = pt.tag_id JOIN tag_groups tg ON tg.id = t.tag_group_id AND tg.key = 'occasion'`, group by `t.key`** (rev 3 — `product_occasions` no longer exists); the local `OCCASION_TH` label map can later be replaced by `t.label`. ⚠ raw SQL is invisible to typecheck — this file needs manual verification in `next dev` after rename. |
...
5. Highest-risk spots: `lib/dresses.ts` mapper rewrite (touches every page), **occasions→tag-system swap inside `listOccasions()`/mapper + the seed-order dependency (tags must exist before products seed `product_tags`)**, `app/admin/metrics/page.tsx` raw SQL (rename + occasion-join rewrite), seller `updateDress` child-table sync + `productTypeId` injection + tag-key resolution, KycPlan value-case change, `logAdminAction` → audit_logs rewrite.
